...
Loop: `select … from analysis_requests where status = 'queued' for update skip locked` → `thesis_consume_quota_for_provider` → Yahoo previous close (D40) into `analysis_evidence` step 0 #1 → native lab completion (D39) → `reports` insert → Playwright PDF into `report-pdfs/{family_id}/{report_id}.pdf`.
