-- =============================================================================
-- Thesis — Migration 020
-- analysis_requests.intended_investment: extra capital the user plans to put
-- into this ticker. Written by thesis_accept_analysis. Does not change lots.
-- =============================================================================

insert into schema_migrations (id, name) values (20, '020_intended_investment.sql');

alter table analysis_requests
  add column if not exists intended_investment numeric(20, 6) not null default 0;

comment on column analysis_requests.intended_investment is
  'Additional amount the user plans to invest in this ticker. Native currency of the request. Never written to holding_lots.';
...
drop function if exists thesis_accept_analysis(
  text, analysis_lens[], numeric, numeric, char(3), analysis_intent, avg_down_policy,
  risk_band, cagr_band, tax_residency, text, text, jsonb, text, numeric
);
