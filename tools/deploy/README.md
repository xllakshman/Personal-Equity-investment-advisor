...
| [`vercel_prod.sh`](vercel_prod.sh) | Set Vercel Production `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY`, plus optional server `RESEND_API_KEY` from `.env.prod`, then `vercel deploy --prod`. |
